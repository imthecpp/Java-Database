...
/**
 * A proxy for a dynamic com.mysql.jdbc.ConnectionConfiguration implementation that load balances requests across a series of MySQL JDBC connections, where the balancing
...
public class LoadBalancedConnectionProxy extends MultiHostConnectionProxy implements PingTarget {
    ...
    /**
     * Creates a proxy for java.sql.ConnectionConfiguration that routes requests between the given list of host:port and uses the given properties when creating connections.
     * 
     * @param hosts
     *            The list of the hosts to load balance.
     * @param props
     *            ConnectionConfiguration properties from where to get initial settings and to be used in new connections.
     * @throws SQLException
     */
    ...
    /**
     * Wraps this object with a new load balanced ConnectionConfiguration instance.
     * 
     * @return
    ...
    synchronized void pickNewConnection() throws SQLException {
        ...
        this.isClosed = true;
        this.closedReason = "ConnectionConfiguration closed after inability to pick valid new connection during load-balance.";
    }
    ...
    /**
     * Proxies method invocation on the java.sql.ConnectionConfiguration interface, trapping "close", "isClosed" and "commit/rollback" to switch connections for load
     * balancing.
    ...
    public synchronized void doPing() throws SQLException {
        ...
        for (Iterator<String> i = this.hostList.iterator(); i.hasNext();) {
            ...
            try {
                ...
                if (host.equals(this.connectionsToHostsMap.get(this.currentConnection))) {
                    ...
                    closeAllConnections();
                    this.isClosed = true;
                    this.closedReason = "ConnectionConfiguration closed because ping of current connection failed.";
                    throw e;
                }
        ...
        if (!foundHost) {
            closeAllConnections();
            this.isClosed = true;
            this.closedReason = "ConnectionConfiguration closed due to inability to ping any active connections.";
