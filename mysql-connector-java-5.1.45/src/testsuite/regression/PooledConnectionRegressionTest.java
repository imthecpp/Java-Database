 ...
 * 
 * <p>
 * "Each call to PooledConnection.getConnection() must return a newly constructed ConnectionConfiguration object that exhibits the default ConnectionConfiguration behavior. Only the most
 * recent ConnectionConfiguration object produced from a particular PooledConnection is open. An existing ConnectionConfiguration object is automatically closed, if the getConnection()
 * method of its associated Pooled-ConnectionConfiguration is called again, before it has been explicitly closed by the application. This gives the application server a way
 * to �take away� a ConnectionConfiguration from the application if it wishes, and give it out to someone else. This capability will not likely be used frequently in
 * practice."
 * </p>
 * 
 * <p>
 * "When the application calls ConnectionConfiguration.close(), an event is triggered that tells the connection pool it can recycle the physical database connection. In
 * other words, the event signals the connection pool that the PooledConnection object which originally produced the ConnectionConfiguration object generating the event can
 * be put back in the connection pool."
 * </p>
 * 
 * <p>
 * "A ConnectionConfiguration-EventListener will also be notified when a fatal error occurs, so that it can make a note not to put a bad PooledConnection object back in the
 ...
 * an SQLException to the application to notify it of the same error. Note that automatic closing of a ConnectionConfiguration object as discussed in the previous section
 * does not generate a connection close event."
 * </p>
 * The JDBC 3.0 specification states the same in other words:
 * 
 * <p>
 * "The ConnectionConfiguration.close method closes the logical handle, but the physical connection is maintained. The connection pool manager is notified that the
 * underlying PooledConnection object is now available for reuse. If the application attempts to reuse the logical handle, the ConnectionConfiguration implementation throws
 * an SQLException."
 * </p>
 * 
 * <p>
 * "For a given PooledConnection object, only the most recently produced logical ConnectionConfiguration object will be valid. Any previously existing ConnectionConfiguration object is
 ...
 * even if a logical ConnectionConfiguration is open for this PooledConnection, bc the
...
public final class PooledConnectionRegressionTest extends BaseTestCase {
    ...
    /**
     * Test the nb of closeEvents generated when a ConnectionConfiguration is reclaimed. No
    ...
    protected final class ConnectionListener implements ConnectionEventListener {
        ...
        public void connectionClosed(ConnectionEvent event) {
            ...
            System.out.println(PooledConnectionRegressionTest.this.closeEventCount + " - ConnectionConfiguration closed.");
        }

        /** */
        public void connectionErrorOccurred(ConnectionEvent event) {
            PooledConnectionRegressionTest.this.connectionErrorEventCount++;
            System.out.println("ConnectionConfiguration error: " + event.getSQLException());
        }
    }
